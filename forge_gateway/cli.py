...
def _generate_yaml() -> tuple[str, list[str], bool]:
    ...
    yaml_text = (
        "# Forge config generated by `forge init` (schema: forge_gateway/settings.py)\n"
        "version: 1\n\n"
        "server:\n"
        "  host: 127.0.0.1\n"
        "  port: 4000\n\n"
...
def cmd_models(args: argparse.Namespace) -> int:
    ...
    if not config.models:
        discovery_providers = [p.name for p in config.providers if p.discovery]
        if discovery_providers:
            if len(discovery_providers) == 1:
                subject, verb = f"provider {discovery_providers[0]!r}", "has"
            else:
                names = ", ".join(repr(n) for n in discovery_providers)
                subject, verb = f"providers {names}", "have"
            print(
                f"Note: {subject} {verb} discovery enabled - models register live at "
                "server start (run forge doctor to see the live count)."
            )
    return 0
...
def cmd_guard(args: argparse.Namespace) -> int:
    if args.max_cost is not None and args.max_cost < 0:
        print("forge: --max-cost must not be negative (use 0 to allow only free models)",
              file=sys.stderr)
        return 1
...
def main(argv: Optional[list[str]] = None) -> int:
    # Windows consoles often default to a legacy codepage (e.g. cp949) that can't
    # encode arbitrary Unicode output; reconfigure to utf-8 so `print`/logging
    # never crashes on non-ASCII text. No-op on platforms/streams without it.
