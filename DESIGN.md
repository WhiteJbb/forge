...
| `GET /v1/stats/recent` | 최근 요청 피드 (대시보드 소비) | UX 스프린트 (U4) |
| `GET /metrics` | Prometheus 포맷 | M3 |
| `GET /dashboard` | 대시보드 데이터 JSON (내장 SPA가 소비) | 유지 |
| `GET /dashboard/ui` | 내장 정적 대시보드 SPA (§5.10) | M3 |
| `POST /admin/reload` | forge.yaml 핫 리로드 + re-discovery | M2 |
| `POST /admin/provider` | **501 — 미구현, `POST /admin/reload`로 대체** (forge.yaml 수정 후 reload) | M2 |
...
27. ~~**Capability 학습 루프** (§5.11-3) — 텔레메트리 점수 보정 + feature 자동 강등 (AI Judge의 실용 대체)~~ → **완료** (`core/tuner.py`)
