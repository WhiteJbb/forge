...
| 13 | LICENSE + README 클라이언트 연동 문서 | §8.2 | 완료 (MIT) |

> **M1 전 항목 완료.** unittest 67건 + 통합 스모크 13항목 통과. 실기동 검증(NVIDIA 키로
> 실제 라우팅)은 사용자 환경에서 확인 필요. 취소 전파는 코드 구현됨, 자동 테스트 미커버.
> 다음: 이 문서를 M2 실행 계획으로 갱신 후 착수.
